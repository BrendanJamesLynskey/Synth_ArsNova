...
class ArsNovaEngine {
    constructor() {
        ...
        // Indices kept modest — the scaffold must stay warm and sit UNDER the
        // sung voices; a gentle per-note lowpass rounds off the FM sidebands.
        this.tenorPreset  = { ratio: 1.0, index: 1.8, wave: 'sine' };
        this.contraPreset = { ratio: 2.0, index: 1.0, wave: 'sine' };
    ...
    createPart(cfg, index) {
        ...
        // Sung parts may sit a few cents apart (their own chorus blurs it); the
        // INSTRUMENTAL scaffold must stay near-true — a sustained tenor even
        // 9 cents off makes every fifth and octave against the singers beat.
        const detuneCents = cfg.vocal
            ? (index - 1.5) * 5 + (Math.random() - 0.5) * 3
            : (index - 1.5) * 1.6 + (Math.random() - 0.5) * 2;
        ...
        if (cfg.vocal) {
            ...
            const singers = detunes.map((cents, di) => {
                const v = VocalVoices.create(this.ctx, {
                    ...
                    breath: 0.06,
                    // Explicit vibrato depth in cents (overrides the library's
                    // ~30 c default): Ars Nova's fast melismatic surface wants
                    // a moderate, early-music vibrato — and the two stacked
                    // folds each carry their OWN uncorrelated vibrato, so deep
                    // settings beat against each other. Triplum a touch
                    // livelier than motetus (cfg.vibCents per role).
                    vibCents: (cfg.vibCents || 17) + di * 2
                });
    ...
    setupVoices() {
        ...
        // The SUNG upper voices carry the piece; the instrumental scaffold sits
        // a few dB under them (it was the other way round — a loud dull FM
        // drone burying the singers). vibCents: triplum slightly livelier.
        const layout = [
            { role: 'tenor',       vocal: false, octave: 0, vol: 0.27, density: 0,   vowels: null                          },
            { role: 'triplum',     vocal: true,  octave: 1, vol: 0.48, density: 3.0, vowels: ['e','i','a'], vibCents: 19   },
            { role: 'motetus',     vocal: true,  octave: 0, vol: 0.42, density: 1.4, vowels: ['o','a','u'], vibCents: 15   },
            { role: 'contratenor', vocal: false, octave: 0, vol: 0.20, density: 0,   vowels: null                          }
        ];
    ...
    scheduleVirelaiPhrase() {
        ...
        // Played SOFT (levelScale) — it is an accompaniment under a solo song,
        // and a full-strength 9-second FM drone buries the singer.
        const tenor = this.parts[0];
        if (tenor && !tenor.vocal) {
            this.playTenorNote(tenor, this.noteFreqIn(1, isB ? 4 : 0, 0, 0), phraseLen * sb * 0.96, 0, 0.55);
        }
        const contra = this.parts.find(p => !p.vocal && p.role === 'contratenor');
        if (contra) {
            this.playTenorNote(contra, this.noteFreqIn(1, isB ? 8 : 4, 0, 0), phraseLen * sb * 0.96, 0, 0.55);
        }
    ...
    playVoiceNote(part, freq, duration, delay, vowel, slideFrom) {
        ...
        // Ramp (not step) into the sustain shelf — a setValueAtTime here put a
        // small amplitude CLICK on every sung note.
        g.linearRampToValueAtTime(peak * 0.92, t0 + Math.max(attack, duration * 0.6));
     ...
     */
    playTenorNote(part, freq, duration, delay, levelScale) {
        if (!isFinite(freq) || freq <= 0 || !isFinite(duration) || duration <= 0) return;
        const t0 = this.ctx.currentTime + (delay || 0);
        const p = part.preset;
        const level = levelScale != null ? levelScale : 1;
        const detune = part.detuneCents + (Math.random() - 0.5) * 2;
        ...
        const bright = 0.45 + this.brightness * 0.7;
        ...
        modGain.gain.linearRampToValueAtTime(peakDev * 1.2, t0 + atk);            // soft bowed swell
        ...
        // --- Gentle lowpass rounds the FM sidebands into a warm wooden tone ---
        const lp = this.ctx.createBiquadFilter();
        lp.type = 'lowpass';
        lp.frequency.value = Math.min(2400, freq * (5 + this.brightness * 5));
        lp.Q.value = 0.4;

        // --- Amplitude ADSR (slow, sustained; ramped shelf — no step clicks) ---
        const amp = this.ctx.createGain();
        const a = Math.min(0.18, duration * 0.25);
        const r = Math.max(0.3, duration * 0.4);
        amp.gain.setValueAtTime(0, t0);
        amp.gain.linearRampToValueAtTime(0.85 * level, t0 + a);
        amp.gain.linearRampToValueAtTime(0.76 * level, t0 + Math.max(a, duration * 0.6));
        amp.gain.exponentialRampToValueAtTime(0.001, t0 + duration + r);
        carrier.connect(lp);
        lp.connect(amp);
        amp.connect(part.busGain);

        // --- Slow vibrato on the long held tenor. The modulator TRACKS the
        // vibrato (scaled by its ratio) so the FM spectrum stays harmonic as
        // the pitch waves — carrier-only vibrato smeared the sidebands into an
        // un-instrumental chorus wobble. ---
        if (duration > 0.6) {
            const vib = this.ctx.createOscillator();
            vib.type = 'sine';
            vib.frequency.value = 4.2 + Math.random() * 0.8;
            const vibDepth = this.ctx.createGain();
            vibDepth.gain.value = freq * 0.003;
            const vibMod = this.ctx.createGain();
            vibMod.gain.value = p.ratio;
            vib.connect(vibDepth);
            vibDepth.connect(carrier.frequency);
            vibDepth.connect(vibMod); vibMod.connect(mod.frequency);
